...
2. **Round Gameplay:** For each of the five sentences:
   - The sentence is displayed in white.
   - A three-second countdown appears before typing starts.
   - The player types the sentence, and the program measures elapsed time.
   - Words per minute (WPM) and accuracy percentage are calculated.
   - Mistakes are highlighted using color-coded feedback.
3. **Session Summary:** After five rounds, the program computes average WPM and accuracy. Achievements such as "Accuracy Master," "Speed Demon," or "Persistent Typist" are awarded based on performance.
4. **Scoreboard:** The player's score is saved in `scores.json` for persistent tracking. A high score table displays all previous scores, highlighting the top three performers.
...
- `main()`: The main loop controlling game flow, handling replay options, and integrating scoring.

## Design Choices
- **Persistent JSON Score Storage:** Tracks player progress across multiple sessions.
- **Color Feedback:** Correct characters in green, mistakes in red for visual clarity.
